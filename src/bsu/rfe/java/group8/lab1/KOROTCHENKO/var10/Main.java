...
public class Main {
    public static void main(String[] args) throws Exception {
        Food[] breakfast = new Food[20];   // Определение ссылок на продукты завтрака
        int countItems = 0;
        ...
        for (String arg: args){
            ...
            if (parts[0].equals("Cheese")){                   // У сыра дополнительных параметров нет
                breakfast[countItems] = new Cheese();
            }
            else if (parts[0].equals("Apple")){               // У яблока – 1 параметр, который находится в parts[1]
                breakfast[countItems] = new Apple(parts[1].toLowerCase());
            }
            else if(parts[0].equals("Lemonade")){             // У лимонада – 1 параметр, который находится в parts[1]
